...
def _match_glob_patterns(
    ...
    for pattern in include_patterns:
        ...
        if not normalized.startswith("/"):
            full_pattern = os.path.join(base_dir, normalized)
        else:
...
def _read_binary_file(file_path: str) -> dict[str, Any]:
    """Read binary file and return as inline data."""
    ext = Path(file_path).suffix.lower()
...
def _read_text_file(file_path: str, max_lines: int = 2000) -> dict[str, Any]:
...
def read_many_files(
    ...
    try:
        ...
        if respect_gemini_ignore:
            geminiignore_path = os.path.join(base_dir, ".geminiignore")
        ...
        # Find matching files
        matched_files = _match_glob_patterns(base_dir, include, exclude_patterns)
        ...
        for file_path in matched_files:
            ...
            if file_type in ("image", "audio", "pdf"):
                ...
                try:
                    binary_data = _read_binary_file(file_path)
                ...
                try:
                    ...
                    if result.get("isTruncated"):
                        file_content += "[WARNING: This file was truncated. To view the full content, use the 'read_file' tool on this specific file.]\n\n"
